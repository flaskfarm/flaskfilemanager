...
def init(app, register_blueprint=True, url_prefix='/fm', access_control_function=None,
    ...
    if register_blueprint:
        ...
        app.register_blueprint(filemanager_blueprint)
...
def copy_file():
    ...
    if os.path.exists(os_new_path):
        #return error('A file with that name (%s) already exists' % os_new_path)
        fname, ext = os.path.splitext(os_new_path)
        os_new_path = os_new_path.replace(fname, fname + ' copy')
        old_name = old_name.replace(fname, fname + ' copy')
